...
function processDirectory(directory) {
  ...
  for (const file of files) {
    ...
    if (stat.isDirectory() && !file.startsWith('.') && file !== 'node_modules') {
      ...
      // Also support imports with extensions (e.g., .jsx).
      const importRegex = /(['"])((?:\.\.\/)*|\.\/|@\/)components\/([a-zA-Z0-9_-]+)(?:\.jsx?)?(['"])/g;
      
      content = content.replace(importRegex, (match, quote1, prefix, compName, quote2) => {
        if (actualMoves[compName]) {
          changed = true;
          // Here we omit the extension if it existed, standardizing it to no extension.
